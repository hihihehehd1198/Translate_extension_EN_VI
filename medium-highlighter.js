// import * as translate from "//translate.google.com/translate_a/element.js?cb=googleTranslateElementInit";
const highlightColor = "rgb(213, 234, 255)";

const template = `
  <template id="highlightTemplate">
    <span class="highlight" style="background-color: ${highlightColor}; display: inline"></span>
  </template>
  <button id="mediumHighlighter">
    <svg class="text-marker" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 544 512"><path d="M0 479.98L99.92 512l35.45-35.45-67.04-67.04L0 479.98zm124.61-240.01a36.592 36.592 0 0 0-10.79 38.1l13.05 42.83-50.93 50.94 96.23 96.23 50.86-50.86 42.74 13.08c13.73 4.2 28.65-.01 38.15-10.78l35.55-41.64-173.34-173.34-41.52 35.44zm403.31-160.7l-63.2-63.2c-20.49-20.49-53.38-21.52-75.12-2.35L190.55 183.68l169.77 169.78L530.27 154.4c19.18-21.74 18.15-54.63-2.35-75.13z"></path></svg>
  </button>
`;

const styled = ({ display = "none", left = 0, top = 0 }) => `
  #mediumHighlighter {
    align-items: center;
    background-color: black;
    border-radius: 5px;
    border: none;
    cursor: pointer;
    display: ${display};
    justify-content: center;
    left: ${left}px;
    padding: 5px 10px;
    position: fixed;
    top: ${top}px;
    width: 40px;
    z-index: 9999;
  }
  .text-marker {
    fill: white;
  }
  .text-marker:hover {
    fill: ${highlightColor};
  }
`;

class MediumHighlighter extends HTMLElement {
  get markerPosition() {
    return JSON.parse(this.getAttribute("markerPosition") || "{}");
  }

  // get styleElement() {
  //   return this.shadowRoot.querySelector("style");
  // }

  get highlightTemplate() {
    return this.shadowRoot.getElementById("highlightTemplate");
  }

  static get observedAttributes() {
    return ["markerPosition"];
  }

  constructor() {
    super();
    this.render();
  }

  render() {
    document.addEventListener("mouseup", () => this.highlightSelection());
  }

  createTooltipElement(elementCurrent, text) {
    const customBackground = document.createElement("div");
    let customWidth = elementCurrent.offsetWidth;
    let customHeight = elementCurrent.offsetHeight;

    customBackground.innerHTML = `
    <span id="customToolTip" style="width:${customWidth}px;
    border:1px solid black; position:absolute;
    top:-${customHeight}px">${text}</span>`;

    elementCurrent.style.position = "relative";
    elementCurrent.appendChild(customBackground);
  }

  createToolTip(el, text) {
    let customWidth = el.offsetWidth - 300 > 0 ? el.offsetWidth - 300 : 300;
    let customHeight = +el.offsetHeight - 500 > 0 ? el.offsetHeight - 500 : 200;

    el.style.position = "relative";
    const customTemplatePopup =
      "width=" + customWidth + ",height=" + customHeight;

    this.newTranslatePopup = window.open(
      "http://vi-translator-extension.s3-website-ap-southeast-1.amazonaws.com/",
      text,
      customTemplatePopup
    );
  }
  newTranslatePopup;
  highlightSelection() {
    try {
      const selection = window.getSelection().toString();

      this.newTranslatePopup?.close();

      document.getElementById("customToolTip")?.remove();
      if (selection === "") {
        this.newTranslatePopup.close();

        document.getElementById("customToolTip")?.remove();
      } else {
        const currentEl = window.getSelection()["focusNode"]?.parentElement;
        this.createToolTip(currentEl, selection);
      }
    } catch (error) {}
  }

  highlightRange(range) {
    const clone =
      this.highlightTemplate.cloneNode(true).content.firstElementChild;
    clone.appendChild(range.extractContents());
    range.insertNode(clone);
  }
}

window.customElements.define("medium-highlighter", MediumHighlighter);
